...
class OnlineDataAcquire(object):
    ...
    def main_thread(self):
        try:
            ...
            N_buffer = int(self.length_buffer * self.fs_eeg)

            logger.debug("start receiving data.")
            while self.is_running:
                eeg.data_chunk, eeg.time_chunk = self.eeg_inlet.pull_chunk(timeout = 0.0)
                ...
                if eeg.time_chunk:
                    ...
                    _, Ns = eeg.data.shape
                    if Ns > N_buffer:
                        eeg.data = eeg.data[:, (Ns-N_buffer):Ns]
                        eeg.time = eeg.time[(Ns-N_buffer):Ns]
            #marker = DataStruct()
            #marker.data = np.empty((0), dtype=np.int64)

            #self.epochs.set(marker = marker) # push by reference
...
class Epochs():
    ...
    def set(self, eeg = None, marker = None):
        if eeg is not None:
            self.eeg = eeg
